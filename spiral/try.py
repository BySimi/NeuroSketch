...
if submit:
    ...
    with st.spinner(text="This may take a moment..."):
        ...
        # Debugging: Check the number of classes in both the prediction and the labels file
        num_classes_in_model = len(prediction[0])
        num_classes_in_labels = len(class_names)
        
        st.write(f"Number of predicted classes: {num_classes_in_model}")
        st.write(f"Number of classes in labels.txt: {num_classes_in_labels}")
        
        if num_classes_in_labels == num_classes_in_model:
            ...
            st.error("The number of classes in the model does not match the labels in labels.txt. Please check both.")
